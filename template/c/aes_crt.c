...
void readMultiFromAddress(uint32_t pAddress, uint32_t *pData, int block_size) {
  for (unsigned int i = 0; i < block_size; ++i)
    pData[(block_size - 1) - i] =
        readFromAddress(pAddress + (i * BYTES_PER_WORD));
}

void writeMultiToAddress(uint32_t pAddress, uint32_t *pData, int block_size) {
  for (unsigned int i = 0; i < block_size; ++i)
    writeToAddress(pAddress + (((block_size - 1) - i) * BYTES_PER_WORD),
                   pData[i]);
}

void writeMulticharToAddress(uint32_t pAddress, char *pData,
                             int block_byte_size) {
  ...
  for (int i = (block_byte_size - 1); i >= 0; i--) {
    ...
    if (i % BYTES_PER_WORD == 0) {
      writeToAddress(
          pAddress + (((block_size - 1) - block_ptr) * BYTES_PER_WORD), wdata);
      block_ptr++;
    }
  }
}
...
int verifyMulti(uint32_t *expectedData, uint32_t *receivedData,
                int block_size) {
  int match = 1;
...
void aes_encrypt(uint32_t *pt, uint32_t *st, uint32_t *ct, uint32_t key_sel) {
  ...
  // Wait to see that START has been asserted to make sure DONE has a valid
  // value
...
int check_aes() {
  ...
  uint32_t key[6] = {0x2b7e1516, 0x28aed2a6, 0xabf71588,
                     0x09cf4f3c, 0x2b7e1516, 0x28aed2a6};
  uint32_t ct[AES_CT_WORDS];
  // uint32_t expectedCt[AES_CT_WORDS] = {0x4fcb8db8, 0x5784a2c1, 0xbb77db7e,
  // 0xde3217ac};
  uint32_t expectedCt[AES_CT_WORDS] = {0x4fcb9ca9, 0x75a691f2, 0xff338e2b,
                                       0xb85460db};
